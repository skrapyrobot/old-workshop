...
const getHandler = async (event, context) => {
    ...
    try {
        let scanResults = await executeScan(
            db_functions_1.dynamoDbClient,
            product_helper_1.getProductScanParameters()
        );
        let outputArray = [];
        if (
            scanResults === null || scanResults === void 0
                ? void 0
                : scanResults.Items
        ) {
            for (let item of scanResults === null || scanResults === void 0
                ? void 0
                : scanResults.Items) {
...
const getSingleProductHandler = async (event, context) => {
    ...
    try {
        ...
        console.log(
            `event.pathParameters.productId : ${event.pathParameters.productId}`
        );
        // event.queryStringParameters = {"name":"test"}
        let scanResults = await executeScan(
            db_functions_1.dynamoDbClient,
            product_helper_1.getProductScanParameters(productId)
        );
        let outputProduct = {};
        if (
            scanResults === null || scanResults === void 0
