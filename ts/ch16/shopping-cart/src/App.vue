...
@Options({
    ...
    mounted() {
        ...
        microEventBus
            ...
            .subscribe(this.handleAddItemToCart);
...
export default class App extends Vue {
    ...
    fetchData() {
        ...
        ajax.getJSON(`http://127.0.0.1:3000/carts/${this.username}`).subscribe(
            (data) => {
    ...
    handleAddItemToCart(data: IItemData) {
        ...
        setTimeout(() => {
            this.loading = true;
            ajax.getJSON(
                `http://127.0.0.1:3000/products/${data.productId}`
            ).subscribe(
                (product) => {
                ...
                (error) => {
                    console.log(
                        `VUE: products/${data.productId} : error ${error}`
                    );
                }
            );
        }, 200);
